...
<?php else: ?>
	$instance-><?= $child->setter() ?>($<?= $child->name ?>);
<?php endif ?>
<?php endforeach ?>
<?php /*
<?php foreach ($node->children as $child): ?>
...
<?php foreach ($node->children as $child): ?>
<?php if (($child->isList ? $child->itemClass : $child->class) !== Token::class): /* tokens or lists of tokens don't have extra validation */ ?>
<?php if ($child->optional): ?>
		if ($this-><?= $child->name ?>)
...
<?php endif ?>
	...
	{
<?php foreach ($node->children as $child): ?>
<?php if ($child->itemClass !== Token::class): /* lists of tokens don't have autocorrect */ ?>
<?php if ($child->isList): ?>
		foreach ($this-><?= $child->name ?> as $t)
			$t->_autocorrect();
<?php elseif ($child->class === Token::class): ?>
<?php if (\count($child->tokenTypes) === 1 && isset(TokenType::AUTOCORRECT[$child->tokenTypes[0]])): ?>
		if (!$this-><?= $child->name ?>)
			$this-><?= $child->setter() ?>(new Token(<?= $child->tokenTypes[0] ?>, <?= var_export(TokenType::AUTOCORRECT[$child->tokenTypes[0]], true) ?>));
<?php endif ?>
<?php else: ?>
		if ($this-><?= $child->name ?>)
