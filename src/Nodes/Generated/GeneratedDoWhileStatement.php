...
trait GeneratedDoWhileStatement
{
	...
	{
		$instance = new self;
	$instance->setDoKeyword($doKeyword);
	$instance->setBlock($block);
	$instance->setWhileKeyword($whileKeyword);
	...
	public function _validate(int $flags): void
	{
		...
		if ($this->doKeyword->getType() !== 158)
			throw ValidationException::invalidSyntax($this->doKeyword, [158]);
		if ($this->whileKeyword->getType() !== 258)
			throw ValidationException::invalidSyntax($this->whileKeyword, [258]);
	...
	{
		if (!$this->doKeyword)
			$this->setDoKeyword(new Token(158, 'do'));
		if ($this->block)
			$this->block->_autocorrect();
		if (!$this->whileKeyword)
			$this->setWhileKeyword(new Token(258, 'while'));
