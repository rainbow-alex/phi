...
trait GeneratedUseStatement
{
	...
	protected function &getChildRef(Node $childToDetach): Node
	{
		...
		if ($this->declarations === $childToDetach)
	...
	public function _validate(int $flags): void
	{
		...
		if ($this->keyword->getType() !== 255)
			throw ValidationException::invalidSyntax($this->keyword, [255]);
		if ($this->type)
		if (!\in_array($this->type->getType(), [186, 148], true))
			throw ValidationException::invalidSyntax($this->type, [186, 148]);
		if ($this->leftBrace)
	...
	{
		if (!$this->keyword)
			$this->setKeyword(new Token(255, 'use'));
		if ($this->prefix)
