...
class FuzzGenerator
{
	 ...
	 */
	public function generate(string $expr)
	{
		$state = [];
		...
		return $this->_generate($expr, $state);
	}

	public function _generate(string $expr, &$state = [])
	{
		$firstI = \PHP_INT_MAX;
		$match = null;
		foreach ($this->rules as $name => $rule)
		...
		if ($match !== null)
		{
			foreach ($this->rules[$match]->generateRhs($state[$match]) as $rhs)
			{
				...
				yield from $this->_generate($subExpr, $state);
			}
		}
		else
		{
